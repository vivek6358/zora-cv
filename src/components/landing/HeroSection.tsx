...
/**
 * Zero-re-render typing simulator.
 * Uses a ref + setTimeout to write chars directly into a span, so the
 * React tree never re-renders during the animation loop.
 */
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (charIdx < line.length) {
        ...
        if (textRef.current) {
          textRef.current.textContent = line.slice(0, charIdx + 1);
        }
...
// Static entrance animation variants — defined once outside component
...
export function HeroSection() {
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden bg-white pt-24 pb-16">
      ...
      {/* Static gradient overlays — no JS, no repaints */}
      <div
        aria-hidden="true"
        ...
        style={{
          background:
            ...
            "radial-gradient(circle at 76% 34%,rgba(6,182,212,0.10),transparent 30%)," +
            "linear-gradient(180deg,rgba(255,255,255,0.62) 0%,rgba(255,255,255,0.86) 62%,rgba(255,255,255,1) 100%)",
        }}
      />
      <div
        aria-hidden="true"
      ...
      {/* Ambient blur orbs — GPU blur, no JS */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="flex flex-col gap-8">
            ...
            {/* CTAs */}
            <motion.div {...fadeUp(0.25)} className="flex flex-wrap gap-4 items-center">
              <Link
                href="/dashboard"
            ...
            {/* Social proof */}
            <motion.div {...fadeUp(0.35)} className="flex flex-wrap items-center gap-6 text-sm">
              <div className="flex -space-x-2">
                {(["bg-indigo-400", "bg-violet-400", "bg-pink-400", "bg-sky-400"] as const).map((c, i) => (
          ...
          >
            {/* 3D Stack Container — single animation loop, GPU-only transforms */}
            <motion.div
              animate={{
                rotateX: [15, 20, 15],
                rotateY: [-20, -28, -20],
            ...
            >
              {/* Back Card */}
              <div
                className="absolute inset-0 bg-slate-100 rounded-2xl shadow-xl border border-slate-200 opacity-40"
              ...
              {/* Middle Card */}
              <div
                className="absolute inset-0 bg-slate-50 rounded-2xl shadow-2xl border border-slate-200 opacity-80"
                style={{ transform: "translateZ(-40px) translateX(20px) translateY(20px)" }}
              >
                <div className="h-24 bg-indigo-100 rounded-t-2xl" />
              </div>
              ...
              >
                ...
                <div className="bg-slate-900 p-6 text-white shadow-sm relative overflow-hidden">
                  <div className="absolute inset-0 opacity-10 bg-[linear-gradient(45deg,transparent_25%,rgba(255,255,255,0.2)_50%,transparent_75%)] bg-[length:4px_4px]" />
                  <motion.div
